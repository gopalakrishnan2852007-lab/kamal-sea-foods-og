...
grid_html = '<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">\n'

for p in products:
    card = f'''<div class="product-card group flex flex-col bg-surface-container-lowest rounded-3xl overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_24px_48px_rgba(45,52,53,0.08)] border border-slate-100 dark:border-slate-800/80">
...
# Regex substitute the old grid
# Find from <div class="bento-grid"> up to the closure just before </section>
pattern = r'<div class="bento-grid">.*?(?=</section>)'
new_content = re.sub(pattern, grid_html + '\n', html_content, flags=re.DOTALL)

# Delete the css rules of .bento-grid safely
pattern_css = r'\s*\.bento-grid\s*{[^}]*}\s*@media[^{]*{[^{]*{[^}]*}[^}]*}\s*@media[^{]*{[^{]*{[^}]*}[^}]*}'
new_content = re.sub(pattern_css, '', new_content, flags=re.DOTALL)

# Also fix the JS observer query: `h1, h2, h3, p, .bento-grid > div` -> `h1, h2, h3, p, .product-card`
new_content = new_content.replace(".bento-grid > div", ".product-card")
